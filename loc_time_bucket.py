...
def BucketByLocation(inFNm, outFNm, location_type="pickup"):
    ...
    with open(outFNm, "w") as outF:
        with open(inFNm, "r") as inF:
            ...
            for line in inF:
                ...
                dt = datetime.strptime(vals[datInd], TIME_FORMAT)
                hour = int(dt.hour / 24)
                time = "%s %d" % (dt.strftime(OUTPUT_TIME_FORMAT), hour)
                bucket = GetBucket((longitude, latitude))
                outF.write("%d,%d,%s\n" % (bucket[0], bucket[1], time))
